...
JSON files consumed by `scripts/seed-curated-pins.ts` to bulk-import the Waymark Curated accounts' pins into Firestore.

## Files (one per Waymark Curated account)
...
| `sourceAuthor` | Toujours `null` pour les Waymark Curated (contenu éditorial, pas de source Insta). |
