# Mappies — Manual Setup Guide

Everything Robin needs to click through in browsers, Apple Developer portal, and Xcode so that when the Mappies code lands, it will compile and Sign in with Apple will work end-to-end.
...
If you want a fresh Expo account for Mappies (recommended for cleaner billing/limits), sign in at https://expo.dev and create one; otherwise keep `robiintesteur`.
...
3. Project name: **`mappies-app`**
   - Firebase will auto-generate a project ID like `mappies-app` or `mappies-app-XXXXX`. Accept whatever it suggests — you don't need it to be pretty.
...
2. **Apple bundle ID:** `com.robinhesse.mappies`
   - Format check: reverse-DNS, all lowercase, no underscores, no spaces. `com.robinhesse.mappies` is valid.
...
3. App nickname: `Mappies iOS` (cosmetic only)
...
7. **Where to put it:** save it to `/Users/robinhesse/Documents/1. GitHub/3. Mappies/GoogleService-Info.plist` (project root).
...
4. Description: `Mappies iOS App`
5. **Bundle ID:** Explicit → `com.robinhesse.mappies` (must match Firebase exactly).
...
3. Description: `Mappies Sign In Service`
4. **Identifier:** `com.robinhesse.mappies.signin`
...
8. Primary App ID: select `com.robinhesse.mappies` (the App ID from 2a).
9. **Domains and Return URLs** — get these from Firebase:
   - In Firebase Console → Authentication → Sign-in method → Apple → expand the config. Firebase shows a callback URL like `https://mappies-app-XXXXX.firebaseapp.com/__/auth/handler`. Copy it.
   - Back in Apple: **Domains:** `mappies-app-XXXXX.firebaseapp.com` (host only, no `https://`).
...
2. Key Name: `Mappies Sign In With Apple Key`
3. Check **Sign in with Apple** → click **Configure** next to it.
4. Primary App ID: `com.robinhesse.mappies` → **Save**.
...
2. Fill in:
   - **Services ID:** `com.robinhesse.mappies.signin` (from 2b)
...
- If you want a separate Expo account for Mappies, run `npx eas-cli logout` then `npx eas-cli register` (or log in as the new user). Decide now — the `owner` field goes in `app.json`.
...
Its `submit.production.ios` block hardcodes `appleId`, `ascAppId`, `appleTeamId` — you won't have `ascAppId` for Mappies until App Store Connect has a listing, so leave that blank or omit `submit` for now.

### 3c. Template `eas.json` for Mappies (drop-in when code arrives)

You don't need to create this file yourself — the code delivery will drop it in. But so you know what to expect at `/Users/robinhesse/Documents/1. GitHub/3. Mappies/eas.json`:
...
Difference from Theater: the `development` profile has `ios.simulator: true` so you can run dev-client builds in the iOS simulator (Theater builds dev on-device). We want simulator-friendly for Mappies while it's early.
...
(Confirmed via `xcrun simctl list runtimes`.) That's fine for Mappies — Sign in with Apple works from iOS 13+ and both these runtimes are far past that.
...
The repo already has `/Users/robinhesse/Documents/1. GitHub/3. Mappies/.mcp.json` configured with three MCP servers: **GitHub**, **Firebase**, and **Context7**. Each needs manual auth/setup before Claude Code can use it.
...
2. Name: `Claude Code Mappies MCP`
3. Expiration: 90 days (or "no expiration" if you want to forget about it).
4. Repository access: **Only select repositories** → pick the Mappies repo (create it first on GitHub if it doesn't exist yet).
...
2. It opens your browser → log in with the Google account that owns the `mappies-app` project (Section 1).
3. Back in terminal: `npx firebase-tools@latest projects:list` — you should see `mappies-app` in the list.
4. Set the default project for the Mappies directory:
   ```
   cd "/Users/robinhesse/Documents/1. GitHub/3. Mappies"
   npx firebase-tools@latest use mappies-app
   ```
...
2. Reopen it in the Mappies directory: `cd "/Users/robinhesse/Documents/1. GitHub/3. Mappies" && claude`
...
| Package | Theater version | Mappies target |
| --- | --- | --- |
...
- [ ] Firebase project `mappies-app` exists, iOS app registered with bundle ID `com.robinhesse.mappies`
- [ ] `GoogleService-Info.plist` downloaded and saved to `/Users/robinhesse/Documents/1. GitHub/3. Mappies/GoogleService-Info.plist`
- [ ] Firestore, Storage, Functions (Blaze plan) all enabled
- [ ] Apple App ID `com.robinhesse.mappies` created with Sign In with Apple capability
- [ ] Apple Services ID `com.robinhesse.mappies.signin` created and configured with Firebase's return URL
...
- [ ] `npx firebase-tools@latest projects:list` shows `mappies-app`
