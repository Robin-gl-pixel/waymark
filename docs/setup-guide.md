# Pinti — Manual Setup Guide

Everything Robin needs to click through in browsers, Apple Developer portal, and Xcode so that when the Pinti code lands, it will compile and Sign in with Apple will work end-to-end.
...
If you want a fresh Expo account for Pinti (recommended for cleaner billing/limits), sign in at https://expo.dev and create one; otherwise keep `robiintesteur`.
...
3. Project name: **`pinti-app`**
   - Firebase will auto-generate a project ID like `pinti-app` or `pinti-app-XXXXX`. Accept whatever it suggests — you don't need it to be pretty.
...
2. **Apple bundle ID:** `com.robinhesse.pinti`
   - Format check: reverse-DNS, all lowercase, no underscores, no spaces. `com.robinhesse.pinti` is valid.
...
3. App nickname: `Pinti iOS` (cosmetic only)
...
7. **Where to put it:** save it to `/Users/robinhesse/Documents/1. GitHub/3. Pinti/GoogleService-Info.plist` (project root).
...
4. Description: `Pinti iOS App`
5. **Bundle ID:** Explicit → `com.robinhesse.pinti` (must match Firebase exactly).
...
3. Description: `Pinti Sign In Service`
4. **Identifier:** `com.robinhesse.pinti.signin`
...
8. Primary App ID: select `com.robinhesse.pinti` (the App ID from 2a).
9. **Domains and Return URLs** — get these from Firebase:
   - In Firebase Console → Authentication → Sign-in method → Apple → expand the config. Firebase shows a callback URL like `https://pinti-app-XXXXX.firebaseapp.com/__/auth/handler`. Copy it.
   - Back in Apple: **Domains:** `pinti-app-XXXXX.firebaseapp.com` (host only, no `https://`).
...
2. Key Name: `Pinti Sign In With Apple Key`
3. Check **Sign in with Apple** → click **Configure** next to it.
4. Primary App ID: `com.robinhesse.pinti` → **Save**.
...
2. Fill in:
   - **Services ID:** `com.robinhesse.pinti.signin` (from 2b)
...
- If you want a separate Expo account for Pinti, run `npx eas-cli logout` then `npx eas-cli register` (or log in as the new user). Decide now — the `owner` field goes in `app.json`.
...
Its `submit.production.ios` block hardcodes `appleId`, `ascAppId`, `appleTeamId` — you won't have `ascAppId` for Pinti until App Store Connect has a listing, so leave that blank or omit `submit` for now.

### 3c. Template `eas.json` for Pinti (drop-in when code arrives)

You don't need to create this file yourself — the code delivery will drop it in. But so you know what to expect at `/Users/robinhesse/Documents/1. GitHub/3. Pinti/eas.json`:
...
Difference from Theater: the `development` profile has `ios.simulator: true` so you can run dev-client builds in the iOS simulator (Theater builds dev on-device). We want simulator-friendly for Pinti while it's early.
...
(Confirmed via `xcrun simctl list runtimes`.) That's fine for Pinti — Sign in with Apple works from iOS 13+ and both these runtimes are far past that.
...
The repo already has `/Users/robinhesse/Documents/1. GitHub/3. Pinti/.mcp.json` configured with three MCP servers: **GitHub**, **Firebase**, and **Context7**. Each needs manual auth/setup before Claude Code can use it.
...
2. Name: `Claude Code Pinti MCP`
3. Expiration: 90 days (or "no expiration" if you want to forget about it).
4. Repository access: **Only select repositories** → pick the Pinti repo (create it first on GitHub if it doesn't exist yet).
...
2. It opens your browser → log in with the Google account that owns the `pinti-app` project (Section 1).
3. Back in terminal: `npx firebase-tools@latest projects:list` — you should see `pinti-app` in the list.
4. Set the default project for the Pinti directory:
   ```
   cd "/Users/robinhesse/Documents/1. GitHub/3. Pinti"
   npx firebase-tools@latest use pinti-app
   ```
...
2. Reopen it in the Pinti directory: `cd "/Users/robinhesse/Documents/1. GitHub/3. Pinti" && claude`
...
| Package | Theater version | Pinti target |
| --- | --- | --- |
...
- [ ] Firebase project `pinti-app` exists, iOS app registered with bundle ID `com.robinhesse.pinti`
- [ ] `GoogleService-Info.plist` downloaded and saved to `/Users/robinhesse/Documents/1. GitHub/3. Pinti/GoogleService-Info.plist`
- [ ] Firestore, Storage, Functions (Blaze plan) all enabled
- [ ] Apple App ID `com.robinhesse.pinti` created with Sign In with Apple capability
- [ ] Apple Services ID `com.robinhesse.pinti.signin` created and configured with Firebase's return URL
...
- [ ] `npx firebase-tools@latest projects:list` shows `pinti-app`
