...
Une app iOS "Pinti" qui transforme le screenshot Instagram en pin sur ma carte perso, sans saisie manuelle.
...
3. Depuis Insta ou Photos, "Partager" → **Pinti (via l'iOS Share Sheet)**
...
6. Quand je suis à Paris → ouvre Pinti → carte → je vois tous les lieux à visiter autour de moi.
...
2. En tant qu'utilisateur, je veux partager un screenshot depuis Photos vers Pinti via le Share Sheet iOS, afin d'éviter d'ouvrir manuellement l'app.
3. En tant qu'utilisateur, je veux pouvoir partager un screenshot vers Pinti depuis n'importe quelle app iOS (Photos, Instagram, Safari…) via une iOS Share Extension native — l'extraction se fait automatiquement, sans configuration.
...
22. En tant que nouvel utilisateur, je veux que le partage vers Pinti fonctionne dès l'installation, sans étape de configuration.
...
- Zero user configuration: as soon as the app is installed, "Pinti" appears in the iOS share sheet for images
- Flow: user taps Share on a screenshot → picks Pinti → the main app opens with `hasShareIntent === true`, routes to `SharedImageScreen`, runs the same `extract` Cloud Function pipeline as the in-app upload, then hands off to `ExtractConfirmScreen`
...
**"Qui a du bon goût dans mon réseau ?"** — Je ne fais pas confiance à TripAdvisor. J'ai confiance en 3-5 amis dont le goût matche le mien. Pinti me montre CE QU'ILS ajoutent, pas ce que le monde ajoute. Insight distinctif vs Insta (feed trop bruyant, non focus lieu) et TripAdvisor (avis d'étrangers).
...
| 6 | Cold start feed | Seed avec 3-4 comptes officiels **@pinti.paris.{cool,culturel,chic,food}** (Paris only en V1) |
| 7 | Curation | Comptes clairement badgés "Pinti Curated" (`isCurated: true`). Robin cure manuellement 15-20 lieux/compte avant launch |
...
| 12 | Onboarding | Splash → Auth → **3 slides pitch** → PickUsername → **Écran "Follow Pinti Curated"** (toggles ON par défaut, opt-out possible) → App |
...
25. En tant que nouvel utilisateur, je veux voir un écran "Suis les comptes Pinti Curated" (toggles ON par défaut) juste après avoir choisi mon username, afin d'entrer dans l'app avec un feed déjà peuplé et pas vide.
...
type User = {
  ...
  isCurated?: boolean        // true pour les comptes officiels @pinti.paris.* — active le badge "Pinti Curated"
...
- `OnboardingSlidesScreen` — 3 slides pitch avant le PickUsername
  ...
  - Slide 2 : "Pinti les transforme en carte, en 4 secondes." (solution + wedge)
...
- `SeededFollowScreen` — juste après username, "Suis les comptes Pinti Curated" avec 3-4 toggles ON par défaut (@pinti.paris.cool/culturel/chic/food)
...
### Curation manuelle des comptes Pinti (parallèle au dev)

**Livrable pré-launch** : 3-4 comptes @pinti.paris.* peuplés avec 15-20 lieux chacun. ~1 jour de curation Robin, à faire pendant le dev.

- **@pinti.paris.cool** — bars cocktails, bistros de quartier, cafés indé, spots low-key
- **@pinti.paris.culturel** — musées, galeries, librairies, cinémas indé, expositions temporaires
- **@pinti.paris.chic** — restos étoilés, palaces, gastronomie haut de gamme
- **@pinti.paris.food** — restos généraliste, si redondance avec cool/chic on peut skip
...
- **J1** : Setup Expo + Firebase project Pinti + copie des seams Theater (Auth, firebase.ts, claude.ts). Auth Sign in with Apple end-to-end. **Livrable J1** : je peux me logger, l'app s'ouvre sur un écran vide.
...
- `src/auth/firebase.ts` → copie, nouvelle config Firebase pour Pinti
