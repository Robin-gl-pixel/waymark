...
Guide for exercising Pinti's social features (follow / feed / block / report / delete cascade) end-to-end, at each level of the testing pyramid.
...
(`alice@pinti.test`, `bob@pinti.test`, …) — set a password from the
...
- SearchUsersScreen: exact `@pinti.paris.cool` returns the profile ; partial match returns empty ; private user returns empty
- UserProfileScreen: shows pins + counts + no interactive actions yet

### #12 Follow + feed
- Follow `@pinti.paris.cool` → `followingCount` = 1 on my profile, `followersCount` = 1 on cool's
...
- Pin appears in my carte with "via @pinti.paris.cool"
